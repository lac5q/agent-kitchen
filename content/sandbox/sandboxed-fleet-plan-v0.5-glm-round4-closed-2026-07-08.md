---
title: Sandboxed Coding-Agent Fleet — Implementation Plan v0.5
date: 2026-07-08
topic: sandbox
...
regen_prompt: Read the round-4 GLM verdict (VALIDATED-WITH-FIXES). Close the §1b gap by adding steps 8-11 (deploy mcp-proxy, hermes-proxy user, network-policy + tokens, systemd unit with HERMES_MCP_TCP_BIND=127.0.0.1:7777 + Limits) matching §1a step 9. Update §1a's systemd unit to include the TCP bind env var.
---
...
## v0.5 → v0.6 (next iteration)

Pending verification on actual boxes. Candidates:
- Execute §1a steps 6-9 on oracle-1 (deploy proxy service, restart, verify)
- Execute §1b steps 8-11 on maeve-u1 (after sudo NOPASSWD lands)
- GLM round-5 against v0.5
