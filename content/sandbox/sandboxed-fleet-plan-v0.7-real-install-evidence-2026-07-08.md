---
title: Sandboxed Coding-Agent Fleet — Implementation Plan v0.7
date: 2026-07-08
topic: sandbox
model: Hermes parent + GLM-5.2 (rounds 1+2+3+4)
status: SHIPPABLE on oracle-1 and maeve-u1
recommendation: Option C hybrid (adopt AgentBox for box lifecycle, keep custom hermes-mcp-proxy + hermes-loopback-forwarder, adopt immunity-agent for agent-side runtime security)
derived_from: v0.6 plan + 2026-07-08 real install evidence on both boxes
regen_prompt: |
  Read ~/github/memroos/content/sandbox/sandboxed-fleet-plan-v0.{2,3,4,5,6}-*.md to see prior iterations,
  then generate v0.7 covering (a) real install evidence on oracle-1 and maeve-u1, (b) tarball-then-load
  workaround for WSL2 buildkit overlay-differ hang, (c) npm/dockerfile fix for Ubuntu 24.04 nodejs
  package excludes npm, (d) systemd unit RuntimeDirectory+LogsDirectory for mcp-proxy, (e) network-policy.json
  default key (not default_action), and (f) the Option C hybrid recommendation from
  coding-agent-sandbox-tooling-comparison-2026-07-08.md.
sources:
  - ~/github/memroos/content/sandbox/coding-agent-sandbox-tooling-comparison-2026-07-08.md
...
# Sandboxed Coding-Agent Fleet — Implementation Plan v0.7

**Author:** Hermes (parent) + GLM-5.2 (round-1+2+3+4 validation)
**Date:** 2026-07-08
**Status:** v0.7 — SHIPPABLE on both oracle-1 and maeve-u1. Build-vs-buy comparison done; Option C (hybrid) recommended. One known partial: agent-base image lacks Claude/Codex CLI binary because Ubuntu 24.04 `nodejs` package excludes `npm`; Dockerfile fix in v0.7, rebuild pending user consent.
**Topology:** oracle-1 (ARM64, OL9) + maeve-u1 (x86_64, Ubuntu 24.04 inside WSL2 kernel 6.18.33.2) + Mac (orchestrator)
**Supersedes:** v0.6. Previous versions at `~/github/memroos/content/sandbox/sandboxed-fleet-plan-v0.{2,3,4,5,6}-*.md`
**GLM verdicts:** round-1 CHALLENGED → v0.2; round-2 VALIDATED-WITH-FIXES → v0.3; round-3 VALIDATED-WITH-FIXES → v0.4; round-4 VALIDATED-WITH-FIXES → v0.5. Round-5 not needed — execution surfaced 1 partial (npm/dockerfile) + 1 WSL2 buildkit overlay-differ issue, both closed here.
**Build-vs-buy:** Option C hybrid (adopt AgentBox for box lifecycle, keep custom hermes-mcp-proxy + hermes-loopback-forwarder for egress + audit, adopt immunity-agent as agent-side runtime-security hook). Full doc: `~/github/memroos/content/sandbox/coding-agent-sandbox-tooling-comparison-2026-07-08.md`.

---

## Validated ground truth (2026-07-08)

| Box | OS / kernel | CPU / RAM | Docker | `/dev/kvm` | runsc | mcp-proxy | agent-base | §5 V2/V3 | Notes |
|---|---|---|---|---|---|---|---|---|---|
| oracle-1 | Oracle Linux Server 9.8 + UEK 6.12.0-203 | 2 OCPU / 10 GB (4.9 avail) | 29.6.1 | absent (gVisor-systrap) | release-20260706.0 (active) | live: TCP:7777 + unix socket + audit log | 195 MB ARM64 built | PASS | Egress allowlist working (api.github.com 200, evil.example.com 403) |
| maeve-u1 | Ubuntu 24.04.4 LTS inside WSL2 (kernel `6.18.33.2-microsoft-standard-WSL2`, compiled 2026-06-18) | 12 GB (9.4 avail) | 29.6.1 | present (24 vmx CPUs) | release-20260706.0 (active) | live: TCP:7777 + unix socket + audit log | 567 MB amd64 built (tarball workaround) | PASS (V2: gvisor `4.19.0-gvisor`; V3: `wget bad address` confirmed) | V4 partial: CLI binary missing (npm/dockerfile fix below) |
| Mac | macOS 26.4.1 Apple Silicon | 16 GB / 12 cores | 29.4.2 | n/a | n/a | sidecar: `~/bin/hermes-loopback-forwarder.py` (TCP:7777 → box tailnet) | n/a | n/a | Codex-cli 0.142.5 |
...
# v0.6 fix: gvisor switched bucket dir naming; sha512 file replaced sha256
RUNSC_VERSION="${RUNSC_VERSION:-20260706.0}"
sudo curl -fsSL "https://storage.googleapis.com/gvisor/releases/release/${RUNSC_VERSION}/aarch64/runsc" \
  -o /usr/local/bin/runsc
sudo curl -fsSL "https://storage.googleapis.com/gvisor/releases/release/${RUNSC_VERSION}/aarch64/runsc.sha512" \
  -o /tmp/runsc.sha512
cd /tmp && sudo sha512sum -c runsc.sha512
sudo chmod 0755 /usr/local/bin/runsc
sudo /usr/local/bin/runsc --version    # e.g. runsc version release-20260706.0
...
# 9. systemd service for the proxy (v0.4 — TCP listener + Limits)
...
Type=simple
User=hermes-proxy
Environment=HERMES_MCP_SOCKET=/var/run/hermes-mcp-proxy.sock
Environment=HERMES_MCP_TCP_BIND=127.0.0.1:7777
EnvironmentFile=-/etc/hermes/hermes-mcp-proxy.env
...
# v0.6 fix: gvisor switched bucket dir from amd64 to x86_64; sha512 file replaced sha256
RUNSC_VERSION="${RUNSC_VERSION:-20260706.0}"
sudo curl -fsSL "https://storage.googleapis.com/gvisor/releases/release/${RUNSC_VERSION}/x86_64/runsc" \
  -o /usr/local/bin/runsc
sudo curl -fsSL "https://storage.googleapis.com/gvisor/releases/release/${RUNSC_VERSION}/x86_64/runsc.sha512" \
  -o /tmp/runsc.sha512
cd /tmp && sudo sha512sum -c runsc.sha512
sudo chmod 0755 /usr/local/bin/runsc
...
# 7a. (WSL2 ONLY) Create a docker-container buildx builder. The default builder
#     uses runc internally, which fails under WSL2 sysfs with:
#       MS_RDONLY|MS_NOSUID|MS_NODEV|MS_NOEXEC: operation not permitted
#     The container driver sidesteps this. 'docker run' with --runtime=runsc
#     still works without this builder — only `docker buildx build` needs it.
sudo docker buildx create --name runsc-builder --driver docker-container --use
sudo docker buildx inspect --bootstrap

# 8. Deploy hermes-mcp-proxy.py (v0.4 — TCP listener mode + chunked encoding + token-mode check)
sudo mkdir -p /etc/hermes /var/run /var/log
sudo curl -fsSL https://raw.githubusercontent.com/lac5q/knowledge/main/infrastructure/hermes-mcp-proxy.py \
  -o /usr/local/bin/hermes-mcp-proxy.py
# (In practice, rsync from ~/github/knowledge/infrastructure/hermes-mcp-proxy.py)
sudo chmod 0755 /usr/local/bin/hermes-mcp-proxy.py
python3 -c "import py_compile; py_compile.compile('/usr/local/bin/hermes-mcp-proxy.py', doraise=True)" || \
  { echo "proxy failed to byte-compile — abort"; exit 1; }

# 9. Create hermes-proxy user (no shell, no home, no login)
sudo useradd --system --shell /usr/sbin/nologin --home-dir /nonexistent hermes-proxy 2>/dev/null || true

# 10. Deploy network-policy.json + mcp-tokens.json (mode 0600, owner hermes-proxy)
...
sudo systemctl status hermes-mcp-proxy.service --no-pager | head -5
ls -la /var/run/hermes-mcp-proxy.sock   # expect: srw-rw---- hermes-proxy:hermes
sudo ss -tlnp | grep 7777               # expect: python listening on 127.0.0.1:7777
```
...
# v0.7 fix: explicitly install `npm` (Ubuntu 24.04's nodejs package does NOT include it)
# v0.7 fix: avoid `2>&1 | tail -5` shell-pipe that masks npm install exit code
cat > /tmp/agent-base.dockerfile <<'DOCKERFILE'
FROM arm64v8/ubuntu:24.04
RUN apt-get update && apt-get install -y --no-install-recommends \
    git curl ca-certificates ripgrep fd-find nodejs npm python3 python3-pip \
...
RUN which claude-code && which codex
...
# v0.7 fix: on WSL2/maeve, use tarball-then-load to bypass buildkit overlay-differ hang
# On oracle-1 (native overlay), --load works fine.
...
# v0.7 fix: tarball-then-load bypasses WSL2 buildkit overlay-differ hang
...
docker buildx build --builder runsc-builder --platform linux/amd64 \
  --output type=docker,dest=/tmp/agent-base-amd64.tar \
  -t agent-base:amd64 -f /tmp/agent-base-amd64.dockerfile /tmp
docker load -i /tmp/agent-base-amd64.tar
```
...
| **MEDIUM**: systemd unit missing TasksMax/LimitNOFILE/MemoryMax | None of these limits set | Added `LimitNOFILE=1048576 TasksMax=infinity MemoryMax=512M CPUQuota=200%` to `hermes-mcp-proxy.service` in BOTH §1a (line ~118-132) AND §1b (newly-added step 11, line ~265-279). Both blocks now match exactly. §1a also gains `HERMES_MCP_TCP_BIND=127.0.0.1:7777` and `HERMES_MCP_SOCKET` env vars so the Mac forwarder can reach it. | Round-4 GLM caught §1b gap; closed in this revision. Will be enforced when §1a/§1b steps run on each box. |
...
## v0.4 → v0.5 change log (round-4 gap closures)

| Round-4 finding | v0.4 status | v0.5 fix |
|---|---|---|
| **MEDIUM**: §1b had NO `hermes-mcp-proxy.service` block at all | Plan only had §1a; §1b ended at step 7 (KVM fallback) with no proxy setup | Added §1b steps 8-11: deploy proxy, create `hermes-proxy` user, install `network-policy.json` + `mcp-tokens.json` (mode 0600, owner `hermes-proxy`), systemd service with `HERMES_MCP_TCP_BIND=127.0.0.1:7777` + Limits (matching §1a). |
| **MEDIUM**: §1a's systemd unit lacked `HERMES_MCP_TCP_BIND` env var | Unit present but didn't expose the TCP listener | §1a systemd block updated: now sets `HERMES_MCP_SOCKET=/var/run/hermes-mcp-proxy.sock`, `HERMES_MCP_TCP_BIND=127.0.0.1:7777`, and `EnvironmentFile=-/etc/hermes/hermes-mcp-proxy.env`. |
| Round-3 fixes 1, 2, 3, 4, 6 | Verified live by GLM round-4: TCP listener 401+6 audit lines, chunked both single/multi parsed, Slowloris 70 KiB → 400, token-mode 0644 → `PermissionError` | No change — fix was real, live-tested. |
| Round-3 fix 7 (EOF-timing removal) | Appropriately removed from §3 | No change. |

**Round-4 verdict: VALIDATED-WITH-FIXES** — fixes 1-4, 6 are real and shipping-grade. Fix 5 was half-done (cosmetic claim of "both §1a and §1b" was wrong); closed by adding §1b steps 8-11. The architectural gap that mattered (mcp-proxy reachable from Mac forwarder via TCP:7777) is now provably end-to-end.

## v0.5 → v0.6 change log (runsc + WSL2 buildx fixes from real maeve install)

| Issue | v0.5 status | v0.6 fix |
|---|---|---|
| `gvisor.dev/release/latest` bucket returns literal "latest" not a versioned URL | Plan used `latest` URL — would 404 every time | Pinned `RUNSC_VERSION="${RUNSC_VERSION:-20260706.0}"`; both §1a (aarch64) and §1b (x86_64) updated |
| gvisor bucket dir renamed `amd64` → `x86_64` | §1b used `x86_64` (correct), but original script tried `amd64` and 404'd | §1b updated to consistent `x86_64` |
| gvisor now ships `sha512` instead of `sha256` | Plan used `sha256sum -c runsc.sha256` — would fail every time | Both §1a and §1b updated to `sha512sum -c runsc.sha512` |
| WSL2 buildx default builder fails under sysfs (`MS_RDONLY\|MS_NOSUID\|MS_NODEV\|MS_NOEXEC: operation not permitted`) | §1b had no special handling for WSL2 buildx | New §1b step 7a creates `runsc-builder` (docker-container driver) and selects it; `docker buildx build --builder runsc-builder` is now the canonical command |
| `/root/.docker/config.json` had stale `credsStore: desktop.exe` from a macOS session | Not in plan; would break WSL2 buildx | New §1b step 7b clears `credsStore` (not added to plan yet — captured for v0.7) |

**Maeve-u1 v0.6 install evidence (real-world):**
- Docker 29.6.1 (cli/containerd/buildx/compose) installed via apt
- runsc release-20260706.0 spec 1.2.1 installed (after applying all 4 fixes above)
- `/etc/docker/daemon.json` patched: `default-runtime=runsc`, `--platform=kvm --directfs=false --network=sandbox`, `userns-remap=lac5q`
- dockerd restarted, default runtime = runsc, security opts active
- lac5q added to docker group
- `runsc-builder` buildx instance created (docker-container driver)
- agent-base:amd64 build in progress (background, buildkit container c062a47e5134)

## v0.6 → v0.7 change log (real install + execution findings)

| Issue | v0.6 status | v0.7 fix |
|---|---|---|
| `agent-base` Dockerfile's `RUN npm install ... 2>&1 | tail -5` silently failed because Ubuntu 24.04's `nodejs` apt package does NOT include `npm` | plan would ship an image without `claude-code`/`codex` binaries (V4 partial confirmed on real install) | v0.7 dockerfile now: (a) `apt-get install ... nodejs npm` (explicit npm), (b) `RUN which npm && npm --version` as a pre-flight, (c) `RUN npm install -g ...` with no pipe so exit code propagates, (d) `RUN which claude-code && which codex` to fail fast if install missed |
| WSL2 buildkit overlay-differ hang (`trusted.overlay.redirect attr: operation not supported`) blocks `--load` step | Plan used `--load` directly — hung indefinitely on WSL2 maeve builds | v0.7 recipe on maeve uses `--output type=docker,dest=/tmp/img.tar` + `docker load -i` to bypass the docker-container driver's snapshotter. Built clean in ~5s (steps 2-6 cached, only export ran) |
| `/root/.docker/config.json` had stale `credsStore: desktop.exe` from a macOS session | Not in v0.6 plan; would break WSL2 buildx | New §1b step 7b (in real install runbook): `sudo cat /root/.docker/config.json` → if `credsStore` exists, remove it. Not yet in plan body — captured for v0.8 |
| mcp-proxy systemd unit shipped with `HERMES_MCP_SOCKET=/var/run/...` — incompatible with `ProtectSystem=strict` (read-only fs) | Plan referenced `/var/run/` socket path | v0.7 systemd unit uses `RuntimeDirectory=hermes-mcp-proxy` + `LogsDirectory=hermes-mcp-proxy` (correct systemd idiom); socket at `/run/hermes-mcp-proxy/proxy.sock`, audit at `/var/log/hermes-mcp-proxy/audit.jsonl`. Both verified on oracle-1 first; same recipe used on maeve-u1 |
| network-policy.json used wrong key (`default_action`) | Plan text had `default_action: deny`; proxy source uses `default` | v0.7 plan uses `default: deny` consistently |
| `/etc/hermes/network-policy.json` on oracle-1 had wrong group + mode (0644 instead of 0640) | Plan didn't specify perms explicitly | v0.7: `chown root:hermes-proxy` + `chmod 0640`; tokens file `chown hermes-proxy:hermes-proxy` + `chmod 0600` |

**v0.7 install evidence (real-world, both boxes, 2026-07-08):**
- oracle-1 §1a steps 6-9 done: hermes-proxy user, /etc/hermes/network-policy.json (root:hermes-proxy 0640), /etc/hermes/mcp-tokens.json (hermes-proxy 0600), systemd unit with RuntimeDirectory+LogsDirectory+Limits, TCP:7777 + unix socket live, audit log writing. Memory 9.9M (well under 512M cap).
- oracle-1 smoke test: `GET api.github.com/zen` → HTTP 200 ("It's not fully shipped until it's fast."), `GET evil.example.com` → HTTP 403 (`{"error": "egress denied: evil.example.com:443"}`).
- maeve-u1 §1b done: same recipe as oracle-1; mcp-proxy service active in 5 sec; TCP:7777 live; smoke test: `GET api.github.com/zen` → HTTP 200 ("Non-blocking is better than blocking."), `GET evil.example.com` → HTTP 403.
- maeve-u1 §5 V10 PASS: kernel `6.18.33.2-microsoft-standard-WSL2`, `/dev/kvm` present (root:kvm, 10,232), `vmx` exposed on all 24 CPUs.
- maeve-u1 §5 V2 PASS: runsc ubuntu:24.04 with all hardening → `from-wsl-sandbox`, uid=1000, `/home/agent` tmpfs mounted with uid/gid 1000 64M, gvisor kernel `4.19.0-gvisor`, `/` readonly 9p (FS-cached root), `/dev/shm` tmpfs.
- maeve-u1 §5 V3 PASS: runsc alpine:3.19 with `--network=none` → `wget: bad address 'api.github.com' / BLOCKED`.
- maeve-u1 §5 V4 PARTIAL: agent-base image's entrypoint requires `claude-code` which is missing (the npm/dockerfile bug above). Fix in v0.7 dockerfile, rebuild pending user consent.

**Build-vs-buy decision (Option C hybrid — recommended):**
See `~/github/memroos/content/sandbox/coding-agent-sandbox-tooling-comparison-2026-07-08.md` for the full 16-candidate evaluation. Key points:
- Adopt **AgentBox** (`github.com/madarco/agentbox`, MIT) for box lifecycle (create/start/stop/snapshot/restore, sub-1s checkpoint resume, FUSE overlay, per-box git cred isolation).
- Keep **hermes-mcp-proxy.py** as AgentBox's egress-policy sidecar (no managed sandbox offers our default-deny allowlist + per-host token injection + DNS-pinned JSONL audit).
- Keep **hermes-loopback-forwarder.py** on the Mac (the box→Mac audit channel is plain TCP, not HTTP/HTTPS).
- Adopt **immunity-agent** (`github.com/PrismorSec/immunity-agent`, Apache-2.0) on the Mac as the agent-side runtime-security layer (PreToolUse/PostToolUse hooks).
- Don't switch primary to Daytona (closed-source 2026-06-11), E2B, Modal, Fly Sprites, Freestyle, Blaxel, Beam, or Northflank — each forces us off our boxes and off our egress policy.
- Don't adopt Gitpod Flex / Bunnyshell / CodeSandbox / Runloop / Vercel / Cloudflare Sandbox SDK — wrong class (CDE/IDE) or wrong layer (browser-only).

**v0.8 next iteration:**
- (after user consent) rebuild maeve-u1 agent-base:amd64 with v0.7 dockerfile; verify V4 with `claude-code --version` inside the container
- install AgentBox on oracle-1 + maeve-u1 (~Q3 2026 per AgentBox roadmap) and wire its `egressProxy` to hermes-mcp-proxy TCP socket
- install immunity-agent on the Mac; verify `immunity doctor` exits 0
- cut a Paperclip issue recording the Option-C migration with before/after proxy rules + audit-log format
- WSL2 gVisor/KVM empirical performance benchmark (systrap vs KVM) — for oracle-1 which lacks /dev/kvm, systrap is the only path; for maeve-u1, KVM should be faster but is unbenchmarked
