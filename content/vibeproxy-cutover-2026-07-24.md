...
- Cloudflare tunnel config: `~/.cloudflared/config.yml` had `vibeproxy.epiloguecapital.com` ingress pointing at `http://localhost:8317` (wrong port + IPv6 mismatch). Repointed to `http://127.0.0.1:8318`.
- App upgraded: 1.8.235 → 1.8.247 (CLIProxyAPI 7.2.80 → 7.2.99). Old app moved to Trash.

### 2. Hermes on main-mac

`~/.claude/settings.json` env block:
- Changed `ANTHROPIC_BASE_URL` from `http://127.0.0.1:8787` (no service listening) to `https://vibeproxy.epiloguecapital.com`
...
Already pointing `customModels` at `https://vibeproxy.epiloguecapital.com/v1` since at least 2026-07-13. No change needed.
...
```bash
sudo chflags -R nouchg,noschg /Users/lcalderon/.pi/agent/models.json
sudo chmod 644 /Users/lcalderon/.pi/agent/models.json
sudo xattr -d com.apple.provenance /Users/lcalderon/.pi/agent/models.json
```
