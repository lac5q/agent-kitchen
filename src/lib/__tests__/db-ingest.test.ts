...
describe('db-ingest: deriveAgentId', () => {
  ...
  it('Test 1: deriveAgentId("-Users-lcalderon-github-agent-kitchen") returns "agent-kitchen"', () => {
    expect(deriveAgentId('-Users-lcalderon-github-agent-kitchen')).toBe('agent-kitchen');
  });

  it('Test 2: deriveAgentId("-Users-lcalderon--paperclip-instances-foo") returns "paperclip"', () => {
    expect(deriveAgentId('-Users-lcalderon--paperclip-instances-foo')).toBe('paperclip');
  });
...
describe('db-ingest: ingestAllSessions and recallByKeyword', () => {
  ...
  it('Test 9: Ingesting a JSONL file creates rows in messages table with correct fields', () => {
    const projectDir = path.join(TEST_MEMORY_PATH, '-Users-lcalderon-github-agent-kitchen');
  ...
  it('Test 10: Re-ingesting same file (unchanged mtime+size) is skipped (0 new rows)', () => {
    const projectDir = path.join(TEST_MEMORY_PATH, '-Users-lcalderon-github-test-project');
  ...
  it('Test 11: recallByKeyword returns results with snippet, project, agent_id, rank', () => {
    const projectDir = path.join(TEST_MEMORY_PATH, '-Users-lcalderon-github-recall-test');
  ...
  it('Test 12: recallByKeyword with phrase-quoted query returns exact phrase matches', () => {
    const projectDir = path.join(TEST_MEMORY_PATH, '-Users-lcalderon-github-phrase-test');
