...
- 🔄 **v8.37 Agent Ownership + Scoped Visibility** — Phases 220-223 (added 2026-08-02; AGENTOWN-01..05 + AGENTVIS-01..09 + AGENTUI-01..04). **Live defect: a reviewer signed in and saw all 59 agents.** `/api/agents` has no `authenticateUser`, no `requireRole` and no owner filter, and `registered_agents` has no `owner_user_id` at all — so there is nothing to filter on. **Core decision: visibility is scope, not rank.** `ROLE_RANK` governs what you may *do*; ownership + space membership governs which agents you may *see*. Making visibility a function of seniority is what produced this bug. Rule: `owned ∪ in-my-spaces ∪ (all if admin)`; unowned agents are admin-only and surfaced as a defect. Research that shaped it: **240 of 260 `space_members` rows are already agents across 4 spaces, and 0 agents sit outside a space** — the scoping primitive exists and is populated; only 3 non-test routes read `registered_agents`, and `listRegisteredAgents`/`getRegisteredAgent` are the chokepoints. Delivers v8.35's P1 and the visibility half of P2. Full roadmap: `.planning/milestones/v8.37-agent-ownership-scoped-visibility-ROADMAP.md`
