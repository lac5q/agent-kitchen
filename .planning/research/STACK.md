...
**Ground truth:** The cron logs to `/tmp/knowledge-curator.log` (verified: `crontab -l` shows `0 2 * * * /Users/lcalderon/github/knowledge/knowledge-curator.sh >> /tmp/knowledge-curator.log 2>&1`).
...
| Path | Files | Source |
|------|-------|--------|
| `/Users/lcalderon/knowledge/gdrive/meet-recordings/` | 105 | Old Drive sync (Gemini Notes format) |
| `/Users/lcalderon/github/knowledge/gdrive/meet-recordings/` | 100 | Phase 5 ingestion output (slugged IDs) |

`collections.config.json` currently points to the **old Drive sync path** (`/Users/lcalderon/knowledge/gdrive/...`). The Phase 5 ingestion script (`personal-ingestion-transcripts.py`) writes to `KNOWLEDGE_ROOT / "gdrive" / "meet-recordings"` where `KNOWLEDGE_ROOT = Path.home() / "github/knowledge"` — i.e., `/Users/lcalderon/github/knowledge/gdrive/meet-recordings/`.
