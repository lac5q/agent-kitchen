...
// Source: verified from /api/knowledge/route.ts
{
  "name": "knowledge",
  "category": "other",
  "basePath": "/Users/lcalderon/github/knowledge"
}
```
...
**Fix required:** Add `"basePath": "/Users/lcalderon/github/knowledge/llm-wiki/wiki"` to the llm-wiki entry in `collections.config.json`.
...
- **Hardcoding `/Users/lcalderon/` in `basePath`:** Use environment variables or relative-from-HOME paths when possible. For now the env var `KNOWLEDGE_BASE_PATH` handles the vault path.
...
| Secrets/env vars | `KNOWLEDGE_BASE_PATH` not set in `.env.local` (file doesn't exist) — API uses hardcoded fallback `~/github/knowledge` | Create `.env.local` with `KNOWLEDGE_BASE_PATH=/Users/lcalderon/github/knowledge` for explicitness, or leave fallback (both work) |
...
**How to avoid:** Add `"basePath": "/Users/lcalderon/github/knowledge/llm-wiki/wiki"` to llm-wiki entry.
...
// Source: verified from live filesystem — wiki pages are in wiki/ not repo root
{
  "name": "llm-wiki",
  "category": "business",
  "basePath": "/Users/lcalderon/github/knowledge/llm-wiki/wiki"
}
```
...
| A3 | `basePath` in collections.config.json should use absolute path with `~` expanded | Code Examples | If the server process doesn't expand `~`, use full `/Users/lcalderon/` path |
