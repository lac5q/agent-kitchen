...
| 2  | The hook is registered in ~/.claude/settings.json under SessionStart | VERIFIED | jq confirms 4 SessionStart groups; 4th group contains `bash /Users/lcalderon/.claude/hooks/mem0-session-preload.sh` with timeout 8 and statusMessage "Loading memory context..." |
...
|------|----|-----|--------|---------|
| ~/.claude/settings.json | ~/.claude/hooks/mem0-session-preload.sh | SessionStart hook command registration | WIRED | `jq '.hooks.SessionStart[].hooks[].command'` returns `"bash /Users/lcalderon/.claude/hooks/mem0-session-preload.sh"` |
...
| Different project uses correct name in header | `echo '{"cwd":"/Users/lcalderon/github/shopifybot","source":"startup",...}' \| bash` | Not run (separate project data) | SKIP — hook behavior (basename extraction) verified in code |
