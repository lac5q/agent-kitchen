...
import { createWriteRule, getWriteRules } from "@/lib/write-rules";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const rules = getWriteRules(getDb(), spaceId);
    return Response.json({ ok: true, rules, timestamp: new Date().toISOString() });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const rule = createWriteRule(getDb(), {
      spaceId,
      dataType: typeof body.dataType === "string" ? body.dataType : typeof body.data_type === "string" ? body.data_type : "",
      targetDocument: typeof body.targetDocument === "string" ? body.targetDocument : typeof body.target_document === "string" ? body.target_document : "",
