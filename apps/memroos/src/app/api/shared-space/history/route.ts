...
import { getSharedToggleHistory } from "@/lib/shared-space";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const history = getSharedToggleHistory(getDb(), spaceId);
    return Response.json({ ok: true, history, timestamp: new Date().toISOString() });
  } catch (error) {
