...
export async function GET(req: NextRequest) {
  ...
  try {
    const isShared = isSpaceShared(getDb(), spaceId);
    return Response.json({ ok: true, isShared, timestamp: new Date().toISOString() });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  const isShared = body.isShared === true || body.is_shared === true;
  const reason = typeof body.reason === "string" ? body.reason : undefined;

  try {
    setSpaceShared(getDb(), { spaceId, isShared, actorId, reason });
    return Response.json({ ok: true, isShared, timestamp: new Date().toISOString() });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
