...
import { asGsdLane, buildGsdResume } from "@/lib/agent/gsd-control";
...
export async function GET(req: NextRequest) {
  ...
  if (!goalId?.trim()) return Response.json({ ok: false, error: "goal_id parameter is required" }, { status: 400 });

  const result = buildGsdResume(getDb(), {
    goalId,
