...
- Name: voice-chat.spec.ts >> Voice & Chat panel — Flow page >> Voice & Chat panel is visible with agent selector
- Location: e2e/voice-chat.spec.ts:12:7
...
Locator: locator('select').filter({ hasText: 'Memroos Floor' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('select').filter({ hasText: 'Memroos Floor' })
